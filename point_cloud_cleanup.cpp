...
int main (int argc, char** argv)
{
    ...
    if (argc < 4)
    {
        std::cout << "Input should be of the form:\n"
                  << "./cleanup_pcd <file> <min range> <max range>" << std::endl;
        return -1;
    }
    double min_range = atof(argv[2]);
    double max_range = atof(argv[3]);

    // Read cloud data from the supplied file
    pcl::PCDReader reader;
    reader.read<pcl::PointXYZ> (argv[1], *src_cloud);
    ...
    pass_z.setFilterLimits(min_range, max_range);
